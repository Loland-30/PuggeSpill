        ...
        <div>
            <section className="mt-24 flex flex-col gap-6 md:mt-32 md:flex-row md:items-center">
                ...
                <div>
                    ...
                    <p className="mt-2 text-2xl text-white/85">Favorite language: {user.favoriteLanguage}</p>
                    <p className="mt-4 text-white/70">Account created: {createdAt}</p>
                </div>
            </section>
                    ...
                    {profileLanguages.map(language => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex items-center gap-2 rounded-full border px-4 py-2 font-semibold transition ${active ? "border-white/40 bg-white/15 text-white" : "border-transparent bg-black/20 text-white/70 hover:bg-white/10 hover:text-white"}`}
                            >
                                {language.flagUrl && (
                                    <img
...
function ProfileStat({ label, value }: { label: string; value: number }) {
    ...
    return (
        <div className={`rounded-lg border ${palette.border} ${palette.card} ${palette.glow} px-8 py-10 text-center`}>
