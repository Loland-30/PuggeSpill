...
function getTrialRank(correctCount: number, questionCount: number, passingCorrect: number) {
    ...
    if (accuracy >= 0.95) return { rank: "S", label: "Trial mastery", color: "#facc15" }
    if (accuracy >= 0.85) return { rank: "A", label: "Excellent control", color: "#4ade80" }
    if (correctCount >= passingCorrect) return { rank: "B", label: "Trial passed", color: "#38bdf8" }
    if (correctCount >= passingCorrect - 2) return { rank: "C", label: "Almost there", color: "#fb923c" }
    return { rank: "D", label: "Try again", color: "#f87171" }
}
