...
export function useZoneRun() {
    ...
    }, [currentEncounterIndex, currentZone, isBossEncounter])
    ...
    }, [advanceEncounter, currentEnemy.hp, streak])

    const onWrongAnswer = useCallback(() => {
        setStreak(0)
    }, [])

    const resetZoneRun = useCallback(() => {
    ...
    const chooseNextZoneForCurrent = useCallback(() => chooseNextZone(currentZone.id), [currentZone.id])
    ...
    return {
        ...
        zonesCleared,
        advanceEncounter,
        chooseNextZone: chooseNextZoneForCurrent,
        onCorrectAnswer,
        onWrongAnswer,
        resetZoneRun
    }
